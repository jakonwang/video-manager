            ...
            <div class="space-y-4">
                ...
                <div class="flex items-center space-x-3">
                    <input type="checkbox" name="use_cos" id="use_cos" value="1" 
                        {{ (old('use_cos', $settings['use_cos'] ?? false)) ? 'checked' : '' }}
                ...
                <div id="cos-settings" class="space-y-4 {{ (old('use_cos', $settings['use_cos'] ?? false)) ? '' : 'hidden' }}">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <!-- Secret ID -->
                        <div class="space-y-1.5">
                            ...
                            <input type="text" name="cos_secret_id" id="cos_secret_id" 
                                value="{{ old('cos_secret_id', $settings['cos_secret_id'] ?? '') }}"
                        ...
                        <div class="space-y-1.5">
                            ...
                            <input type="password" name="cos_secret_key" id="cos_secret_key" 
                                value="{{ old('cos_secret_key', $settings['cos_secret_key'] ?? '') }}"
                                class="w-full px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200">
                        </div>
                        ...
                        <div class="space-y-1.5">
                            ...
                            <select name="cos_region" id="cos_region"
                                class="w-full px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200">
                                <option value="ap-beijing" {{ (old('cos_region', $settings['cos_region'] ?? 'ap-beijing') == 'ap-beijing') ? 'selected' : '' }}>北京 (ap-beijing)</option>
                                <option value="ap-shanghai" {{ (old('cos_region', $settings['cos_region'] ?? 'ap-beijing') == 'ap-shanghai') ? 'selected' : '' }}>上海 (ap-shanghai)</option>
                                <option value="ap-guangzhou" {{ (old('cos_region', $settings['cos_region'] ?? 'ap-beijing') == 'ap-guangzhou') ? 'selected' : '' }}>广州 (ap-guangzhou)</option>
                                <option value="ap-singapore" {{ (old('cos_region', $settings['cos_region'] ?? 'ap-beijing') == 'ap-singapore') ? 'selected' : '' }}>新加坡 (ap-singapore)</option>
                                <option value="ap-hongkong" {{ (old('cos_region', $settings['cos_region'] ?? 'ap-beijing') == 'ap-hongkong') ? 'selected' : '' }}>香港 (ap-hongkong)</option>
                                <option value="ap-tokyo" {{ (old('cos_region', $settings['cos_region'] ?? 'ap-beijing') == 'ap-tokyo') ? 'selected' : '' }}>东京 (ap-tokyo)</option>
                                <option value="ap-seoul" {{ (old('cos_region', $settings['cos_region'] ?? 'ap-beijing') == 'ap-seoul') ? 'selected' : '' }}>首尔 (ap-seoul)</option>
                            </select>
                        </div>
                        ...
                        <div class="space-y-1.5">
                            ...
                            <input type="text" name="cos_bucket" id="cos_bucket" 
                                value="{{ old('cos_bucket', $settings['cos_bucket'] ?? '') }}"
                                class="w-full px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200">
                        </div>
                        ...
                        <div class="space-y-1.5">
                            ...
                            <input type="url" name="cos_domain" id="cos_domain" 
                                value="{{ old('cos_domain', $settings['cos_domain'] ?? '') }}"
                        ...
                        <div class="space-y-1.5">
                            ...
                            <input type="number" name="cos_timeout" id="cos_timeout" 
                                value="{{ old('cos_timeout', $settings['cos_timeout'] ?? 60) }}"
                                min="10" max="600"
                                class="w-full px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200">
                        </div>
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    testButton.addEventListener('click', function() {
        ...
        // 发送测试请求
        fetch('{{ route("admin.settings.test-cos") }}', {
            method: 'POST',
            headers: {
