    ...
    <div class="glass-effect rounded-xl p-4 shadow-md">
        <div class="flex items-center justify-between">
            <div class="flex items-center space-x-3">
                ...
                <div>
                    <h1 class="text-xl font-bold text-gray-900 dark:text-white">{{ __('admin.edit_category') }}</h1>
                    <p class="text-sm text-gray-600 dark:text-gray-400">编辑视频分类信息</p>
                </div>
            </div>
        </div>
    </div>
        ...
        <form action="{{ route('admin.video-categories.update', $category) }}" method="POST">
            ...
            <div class="mb-6">
                <label for="slug" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                    <i class="fas fa-link mr-1.5 text-primary-500 text-xs"></i>{{ __('admin.slug') }}
                </label>
                <input type="text" name="slug" id="slug" value="{{ old('slug', $category->slug) }}" required
                    class="block w-full rounded-lg border-0 py-3 px-4 text-gray-900 dark:text-white ring-1 ring-inset ring-gray-300 dark:ring-gray-600 placeholder:text-gray-400 focus:ring-2 focus:ring-primary-500 dark:bg-gray-800/50 transition-all duration-200 text-sm"
                    placeholder="{{ __('admin.enter_slug') }}">
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ __('admin.slug_help') }}</p>
                @error('slug')
                <p class="mt-1 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-6">
                ...
                <textarea id="description" name="description" rows="3"
                    ...
                    placeholder="{{ __('admin.enter_description') }}">{{ old('description', $category->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex items-center justify-between pt-6 border-t border-gray-200 dark:border-gray-700">
                ...
                <button type="submit" 
                    ...
                    <i class="fas fa-save mr-1.5 text-xs"></i>{{ __('admin.update_category') }}
                </button>
            </div>
        </form>
    </div>
