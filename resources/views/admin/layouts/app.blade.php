<!DOCTYPE html>
<html lang="{{ app()->getLocale() }}" class="h-full" x-data="{ 
    darkMode: localStorage.getItem('darkMode') === 'true',
    currentLang: localStorage.getItem('adminLang') || '{{ app()->getLocale() }}',
    sidebarOpen: true
...
<head>
    ...
    <script>
        ...
        window.__ = function(key, replace = {}) {
            const translations = {
                'admin.language': '{{ __('admin.language') }}',
                'admin.admin_panel': '{{ __('admin.admin_panel') }}',
                'admin.form_validation_error': '{{ __('admin.form_validation_error') }}',
                'admin.light_mode': '{{ __('admin.light_mode') }}',
                'admin.dark_mode': '{{ __('admin.dark_mode') }}',
                'admin.video_management_system': '{{ __('admin.video_management_system') }}',
                'admin.video_list': '{{ __('admin.video_list') }}',
                'admin.upload_video': '{{ __('admin.upload_video') }}',
                'admin.category_management': '{{ __('admin.category_management') }}',
                'admin.system_settings': '{{ __('admin.system_settings') }}',
                'admin.logout': '{{ __('admin.logout') }}',
                'admin.chinese': '{{ __('admin.chinese') }}',
                'admin.english': '{{ __('admin.english') }}',
                'admin.vietnamese': '{{ __('admin.vietnamese') }}'
            };
        ...
        <aside class="fixed inset-y-0 left-0 z-50 w-64 bg-gradient-to-b from-gray-900 to-gray-800 dark:from-gray-800 dark:to-gray-900 shadow-xl transform transition-transform duration-300 ease-in-out" :class="{'translate-x-0': sidebarOpen, '-translate-x-full': !sidebarOpen}">
            ...
            <div class="flex items-center justify-between h-16 px-4 border-b border-gray-700/50">
                <div class="flex items-center space-x-3">
                    ...
                    <h1 class="text-lg font-semibold text-white">{{ getSiteName() }}</h1>
                </div>
            ...
            <nav class="px-2 py-4 space-y-1">
                <a href="{{ route('admin.dashboard') }}" class="flex items-center px-4 py-3 text-sm font-medium rounded-lg transition-all duration-200 {{ request()->routeIs('admin.dashboard') ? 'bg-indigo-500/10 text-indigo-400' : 'text-gray-300 hover:bg-gray-700/50 hover:text-white' }}">
                    <i class="fas fa-home w-5 h-5 mr-3"></i>
                    <span>{{ __('admin.dashboard') }}</span>
                </a>
                
                <a href="{{ route('admin.videos.index') }}" class="flex items-center px-4 py-3 text-sm font-medium rounded-lg transition-all duration-200 {{ request()->routeIs('admin.videos.*') ? 'bg-indigo-500/10 text-indigo-400' : 'text-gray-300 hover:bg-gray-700/50 hover:text-white' }}">
                    <i class="fas fa-video w-5 h-5 mr-3"></i>
                    <span>{{ __('admin.video_management') }}</span>
                </a>

                <a href="{{ route('admin.video-categories.index') }}" class="flex items-center px-4 py-3 text-sm font-medium rounded-lg transition-all duration-200 {{ request()->routeIs('admin.video-categories.*') ? 'bg-indigo-500/10 text-indigo-400' : 'text-gray-300 hover:bg-gray-700/50 hover:text-white' }}">
                    <i class="fas fa-folder w-5 h-5 mr-3"></i>
                    <span>{{ __('admin.category_management') }}</span>
                </a>

                <a href="{{ route('admin.downloads.index') }}" class="flex items-center px-4 py-3 text-sm font-medium rounded-lg transition-all duration-200 {{ request()->routeIs('admin.downloads.*') ? 'bg-indigo-500/10 text-indigo-400' : 'text-gray-300 hover:bg-gray-700/50 hover:text-white' }}">
                    <i class="fas fa-download w-5 h-5 mr-3"></i>
                    <span>{{ __('admin.download_logs') }}</span>
                </a>

                <a href="{{ route('admin.users.index') }}" class="flex items-center px-4 py-3 text-sm font-medium rounded-lg transition-all duration-200 {{ request()->routeIs('admin.users.*') ? 'bg-indigo-500/10 text-indigo-400' : 'text-gray-300 hover:bg-gray-700/50 hover:text-white' }}">
            ...
            <div class="absolute bottom-0 left-0 right-0 p-4 border-t border-gray-700/50">
                <div class="flex items-center mb-4 px-4">
                    ...
                    <div class="ml-3">
                        @auth
                            ...
                            <p class="text-xs text-gray-400">{{ __('admin.role_admin') }}</p>
                        @else
                            <p class="text-sm font-medium text-white">{{ __('admin.not_logged_in') }}</p>
                            <p class="text-xs text-gray-400">{{ __('admin.please_login') }}</p>
                        @endauth
                    </div>
                </div>
                ...
                @else
                    <a href="{{ route('login') }}" class="w-full flex items-center justify-center px-4 py-3 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-lg transition-colors duration-200">
            ...
            <div class="sticky top-0 z-40 flex h-14 shrink-0 items-center gap-x-2 border-b border-white/10 glass-effect px-3 shadow-xl sm:gap-x-4 sm:px-4 lg:px-6 animate-fade-in">
                ...
                <div class="flex flex-1 gap-x-3 self-stretch lg:gap-x-4">
                    <div class="flex flex-1 items-center justify-between">
                        <h1 class="text-lg font-bold bg-gradient-to-r from-gray-900 to-gray-600 dark:from-white dark:to-gray-300 bg-clip-text text-transparent animate-fade-in ml-2">@yield('title', __('admin.admin_panel'))</h1>
                        ...
                        <div class="flex items-center space-x-3">
                            ...
                            <div class="relative" x-data="{ langOpen: false }">
                                ...
                                <div x-show="langOpen" 
                                    ...
                                    <a href="{{ route('language.switch', ['locale' => 'zh']) }}" 
                ...
                @if($errors->any())
                    <div class="mb-4 rounded-xl glass-effect p-4 transform transition-all duration-500 animate-bounce-in" 
                        ...
                        <div class="flex items-start">
                            ...
                            <div class="ml-4 flex-1">
                                <h3 class="text-sm font-semibold text-red-800 dark:text-red-200">{{ __('admin.form_validation_error') }}</h3>
