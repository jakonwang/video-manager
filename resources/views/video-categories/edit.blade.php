...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-3xl mx-auto">
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-3xl font-bold text-gray-900 dark:text-white">编辑视频分类</h1>
        ...
        <div class="bg-white dark:bg-gray-800 shadow-xl rounded-lg overflow-hidden">
            <form action="{{ route('video-categories.update', $videoCategory) }}" method="POST" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        <label for="slug" class="block text-sm font-medium text-gray-700 dark:text-gray-300">分类别名</label>
                        <input type="text" name="slug" id="slug" value="{{ old('slug', $videoCategory->slug) }}" required
                            class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50 dark:bg-gray-700 dark:text-white"
                            placeholder="输入分类别名">
                        @error('slug')
                        <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="3"
                            class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50 dark:bg-gray-700 dark:text-white"
                            placeholder="输入分类描述">{{ old('description', $videoCategory->description) }}</textarea>
                        @error('description')
                        <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <div class="mt-1 relative rounded-md shadow-sm">
                            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                <i class="{{ $videoCategory->icon ?: 'fas fa-icons' }} text-gray-400"></i>
                            </div>
                            <input type="text" name="icon" id="icon" value="{{ old('icon', $videoCategory->icon) }}"
                    ...
                    <div>
                        ...
                        <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $videoCategory->sort_order) }}"
                            class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50 dark:bg-gray-700 dark:text-white"
                            min="0">
                        @error('sort_order')
                        <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    ...
                    <div class="flex items-center">
                        <input type="checkbox" name="is_active" id="is_active" value="1" {{ $videoCategory->is_active ? 'checked' : '' }}
